...
# X_train = np.load('/data/yangjj_data/luna16_3d_nodules/40mm_datasets/train_x.npy')
# X_train = X_train.reshape(-1,1,26,40,40)
# Y_train = np.load('/data/yangjj_data/luna16_3d_nodules/40mm_datasets/train_y.npy')
X_test = np.load('/data/yangjj_data/luna16_3d_nodules/40mm_datasets/test_x.npy')
X_test = X_test.reshape(-1,1,26,40,40)
Y_test = np.load('/data/yangjj_data/luna16_3d_nodules/40mm_datasets/test_y.npy')
...
model = Sequential()
model.add(Conv3D(nb_filters[0],kernel_size=(3,5,5),padding='same',input_shape=(1, 26, patch_size, patch_size), activation='relu'))
...
# load checkpoint
model.load_weights('model_save/40mm-weights-32-0.98.hdf5')
# filepath="model_save/40mm-weights-{epoch:02d}-{val_acc:.2f}.hdf5"
...
with open("model_40mm.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights("model_40mm.hdf5")
