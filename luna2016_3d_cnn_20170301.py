...
X_train = np.load('transpose_x_train.npy')
X_train = X_train.reshape(-1,1,6,20,20)
Y_train = np.load('transpose_y_train.npy')
X_test = np.load('transpose_x_test.npy')
X_test = X_test.reshape(-1,1,6,20,20)
Y_test = np.load('transpose_y_test.npy')
...
model = Sequential()
model.add(Convolution3D(nb_filters[0],kernel_dim1=nb_conv[0], kernel_dim2=nb_conv[0], kernel_dim3=nb_conv[0],border_mode='same',input_shape=(1, 6, patch_size, patch_size), activation='relu'))
model.add(MaxPooling3D(pool_size=(nb_pool[0], nb_pool[0], nb_pool[0])))
model.add(Dropout(0.5))
model.add(Convolution3D(nb_filters[1],kernel_dim1=nb_conv[1], kernel_dim2=nb_conv[1], kernel_dim3=nb_conv[1], border_mode='valid',activation='relu'))
